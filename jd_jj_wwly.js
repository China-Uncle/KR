...
const iliIl = $.isNode() ? require("./jdCookie.js") : "",
      ...
      Il1I = require("./function/h5st41.js");
...
if ($.isNode()) {
  ...
  iii1i = [$.getdata("CookieJD"), $.getdata("CookieJD2"), ...lIiII($.getdata("CookiesJD") || "[]").map(ii1i1i => ii1i1i.cookie)].filter(IiIi11 => !!IiIi11);
}
      ...
      if ($.isNode()) {
        if (!(process.env.HELP_JOYPARK && process.env.HELP_JOYPARK == "false")) {
          ...
          if ($.kgw_invitePin && $.kgw_invitePin.length) {
            ...
            if (iIl1I) {
              if (!(iIl1I.helpState && iIl1I.helpState === 1)) {
                if (!(iIl1I.helpState && iIl1I.helpState === 3)) {
                  !(iIl1I.helpState && iIl1I.helpState === 2);
                }
              }
            }
...
async function llII1i(li11ii, I1iiIi) {
  ...
  if (li11ii.length !== 0 && I1iiIl.length !== 0) {
    ...
    $.JOY_COIN_MAXIMIZE && (await illI1I(I1iiIl), await $.wait(parseInt(Math.random() * 2000 + 1000, 10)));
  }
}
...
async function ilI1Ii(lIl1Ii) {
  if (lIl1Ii.filter(I1IIll => I1IIll.joyDTO).length === 0) {
    $.log("工位清理完成！");
    return true;
  }
...
async function ll11Il(IIil, IIii) {
  ...
  return new Promise(IIll => {
    $.post(IiIi1I(I1i1i), async (iiiIll, iilliI, iII1i) => {
      try {
        if (iiiIll) {
          console.log("" + JSON.stringify(iiiIll));
    ...
    $.post(IiIi1I(Iil1i), async (l1lII1, iilllI, i1i1iI) => {
      try {
        if (l1lII1) {
          ...
          $.log("回收🐶 " + (i1i1iI.success ? "成功！" : "失败！【" + i1i1iI.errMsg + "】 code=" + i1i1iI.code));
        }
      } catch (l1I1i) {
        $.logErr(l1I1i, iilllI);
      } finally {
...
async function iii1I() {
  return new Promise(async IlIii => {
    ...
    $.post(IlillI, async (I111l1, iIliII, iIii1I) => {
      try {
        if (I111l1) {
          console.log("" + JSON.stringify(I111l1));
...
function iliII(I111i1, lIl1i1, l1Ilil, iIili) {
  return new Promise(IIil11 => {
    ...
    $.post(liIIIi, async (l1IllI, lIi1Ii, iiilII) => {
      try {
        if (l1IllI) {
          console.log("" + JSON.stringify(l1IllI));
          console.log($.name + " API请求失败，请检查网路重试");
        } else {
          if (safeGet(iiilII)) {
            iiilII = $.toObj(iiilII);
            ...
            if (iiilII.code === 0) {
              ...
              console.log("提现现金：异常:" + JSON.stringify(iiilII));
            }
          }
        }
      } catch (ii1iI1) {
...
async function iillIi(i11III, ii1iIl) {
  try {
    ...
    await l1iii1.genAlgo();
    body = await l1iii1.genUrlParams(ii1iIl.functionId, ii1iIl.body);
    return body;
  } catch (Ii111I) {}
}
...
function iliI1(i1I11I, Ii1lli = {}) {
  ...
  for (let Ii1llI of Ii1lI) {
    ...
    if (i1I11i && typeof i1I11i === "object") {
      i1I11i = JSON.stringify(i1I11i);
    }

    if (i1I11i && Ii1lli.encode) {
      i1I11i = encodeURIComponent(i1I11i);
    }
...
function i11iI(li11Il) {
  ...
  for (i = 0; i < li11Il; i++) {
    Ii1ll1 += IIlIl1.charAt(Math.floor(Math.random() * liII1l));
  }
